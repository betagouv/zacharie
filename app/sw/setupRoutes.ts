...
export function setupRoutes() {
  ...
  if (import.meta.env.PROD) {
    ...
    if (useStaticRoutes.length) {
      ...
      registerRoute(
        ({ request, sameOrigin, url }) =>
          request.destination === "document" && sameOrigin && staticRoutesRegexp.test(url.pathname),
    ...
    if (useDynamicRoutes.length) {
      ...
      registerRoute(
        ({ request, sameOrigin, url }) =>
          request.destination === "document" && sameOrigin && dynamicRoutesRegexp.test(url.pathname),
        new NetworkOnly({
          plugins: [
            {
