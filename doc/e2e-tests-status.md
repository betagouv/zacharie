...
Generated 2026-04-20. 127 specs across 9 folders.

## Results: 94 pass, 31 skip, 1 fail (collecteur/63)

---

## Test inventory

### A. Examinateur (`tests/examinateur/`) — 17 pass, 6 skip
...
| 06 | brouillon-persistance | **skip** | Draft fiche visibility in list unclear |
| 07 | edition-carcasse-depuis-detail | **skip** | Carcasse detail edit flow needs walkthrough |
| 08 | examinateur-pd-self-handoff | **skip** | Entity selector for association de chasse unknown |
| 09 | onboarding-complet | **skip** | Onboarding form selectors need verification |
...
### B. Premier detenteur (`tests/premier-detenteur/`) — 13 pass, 6 skip
...
| 36 | refus-carcasse-avant-transmission | **skip** | PD cannot refuse carcasses — only ETG/collecteur can |
...
### C. ETG (`tests/etg/`) — 16 pass, 2 skip
...
| 48 | vue-carcasses-agregee | **skip** | No CarcasseIntermediaire in ETG seed |
...
| 54 | partage-de-mes-donnees | pass | Opt-in/out |
...
| 58 | acces-cross-etg-refuse | pass | Erreur 500 (should be 403 — see bugs) |
...
### D. Collecteur (`tests/collecteur/`) — 7 pass, 4 skip, 1 fail

| # | Test | Status | Description |
|---|------|--------|-------------|
| 62 | reception-fiche-depuis-pd | pass | "Je controle et transporte" button |
| 63 | transmission-a-etg | **FAIL** | Transmettre stays disabled — unclear why |
| 64 | vue-carcasses-agregee | **skip** | No CarcasseIntermediaire in seed |
| 65 | refus-carcasse-en-collecte | **skip** | Collecteur refusal UX unknown |
...
| 72 | fiche-attribuee-autre-collecteur | pass | Erreur 500 (should be 403) |
| 73 | hors-ligne-prise-en-charge | **skip** | Offline flow deferred |

### E. SVI (`tests/svi/`) — 11 pass, 4 skip
...
| 87 | fiche-sans-carcasse-a-inspecter | **skip** | ETG_REFUSED seed needs enrichment |
| 88 | svi-autre-departement | pass | SVI 2 can't access SVI 1's fiche |
...
| 90 | prise-en-charge | **skip** | No CTA — circuit court is passive view only |
...
| 106 | service-worker-cache | **skip** | Deferred |
...
| 116 | chain-refus-partiel-svi | **skip** | Multi-decision SVI flow needs walkthrough |

### I. Dispatch isolation (`tests/dispatch-isolation/`) — 7 pass, 5 skip
...
| 120 | vue-agregee-etg-carcasses | **skip** | App limitation: aggregate view doesn't filter by branch |
...
| 124 | deux-chaines-svi-paralleles | **skip** | Needs second SVI user in seed |
...
## Skipped tests needing developer input

### Need UI walkthrough / screenshots
- **06 (brouillon)**: Does a draft (unsent) fiche appear in the chasseur list?
- **07 (edition carcasse detail)**: What's the full edit flow on `/chasseur/carcasse/:fei/:id`?
- **08 (self-handoff)**: How does the entity selector work when examinateur assigns to their own association?
- **09 (onboarding complet)**: What are the exact form field selectors for each onboarding step?
- **65 (collecteur refus)**: How does a collecteur refuse a carcasse mid-transport?

### Need seed enrichment
- **48, 64 (ETG/collecteur vue carcasses)**: Seed needs CarcasseIntermediaire records (carcasses "taken in" by the intermediaire)
- **55 (ETG post-SVI)**: SVI_CLOSED seed needs per-intermediaire data
- **87 (SVI fiche sans carcasse)**: ETG_REFUSED seed needs to direct the fiche to SVI with all carcasses refused
- **124 (deux chaines SVI paralleles)**: Seed needs a second SVI user linked to SVI 2 entity

### Need API-level test tooling
- **121, 128**: Direct API calls to test CarcasseIntermediaire endpoint isolation

### By design / deferred
- **33 (partage donnees)**: Page empty without admin API key approval in seed — complex admin setup
- **36 (PD refus carcasse)**: PD cannot refuse carcasses — only ETG/collecteur can. Not a bug.
- **90 (CC prise en charge)**: Circuit court has no CTA — passive view only. By design.
- **106 (SW cache)**: Service worker testing deferred
- **116 (chain refus partiel SVI)**: Multi-decision SVI inspection needs live walkthrough

---

## Bugs discovered

### 1. Cross-role direct URL access returns Erreur 500 instead of 403

**Severity**: Medium (security UX, not data leak — data is not shown)

When a user accesses a fiche URL that belongs to another entity of the same role, the app crashes with Erreur 500 instead of returning a clean 403 or "fiche introuvable" message.

Reproduced in:
- ETG 2 visits `/app/etg/fei/ZACH-...-165242` (assigned to ETG 1)
- Collecteur visits `/app/collecteur/fei/ZACH-...-165242` (ETG fiche)
- SVI 2 visits `/app/svi/fei/ZACH-...-185242` (assigned to SVI 1)

### 2. `/etg/carcasses` aggregate view doesn't filter by dispatch branch

**Severity**: Medium (data leak between intermediaires of the same fiche)

When PD dispatches 2 carcasses to ETG 1 and 2 carcasses to ETG 2, the `/etg/carcasses` aggregate page shows all 4 carcasses to both ETGs. The fiche-level view (`/etg/fei/:numero`) correctly shows only assigned carcasses, but the aggregate view doesn't apply the same filter.

### 3. Collecteur "Transmettre la fiche" stays disabled

**Severity**: Unknown — could be a real bug or a missing test step.

On fiche `ZACH-...-175242` (COLLECTEUR_PRO seed), after collecteur takes charge ("Je controle et transporte"), sets date, selects ETG 1 as prochain detenteur, and selects "Pas de stockage", the "Transmettre la fiche" button remains disabled/greyed out.
...
- **Anomalies are on the detail page, not the creation form**: `/chasseur/carcasse/:fei/:id` has the anomaly referentiel tree modals.
...
- **Curly apostrophes in DSFR labels**: French text uses `'` (U+2019) not `'` — use regex patterns instead of exact string matches.
...
- **Multiple specs running sequentially can crash dev servers**: `ERR_CONNECTION_REFUSED` appears when many specs run back-to-back. Each spec passes individually. Pre-existing infra issue.
