...
test('Onboarding partiel — coordonnées OK mais pas de formation : ChasseurDeactivated affiché', async ({
  page,
}) => {
  ...
  await expect(page.getByText(/Merci pour votre inscription|compléter votre profil/i).first()).toBeVisible({
    timeout: 10000,
  });
});
