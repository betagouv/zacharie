...
import { ApiKeyApprovalStatus, ApiKeyScope, Entity, EntityRelationType, EntityTypes, Prisma, User, UserRoles } from '@prisma/client';
...
router.post(
  '/user/nouveau',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminNewUserDataResponse>, next: express.NextFunction) => {
    const body = req.body;
...
router.get(
  ...
  catchErrors(async (req: express.Request, res: express.Response<AdminUserDataResponse>, next: express.NextFunction) => {
    ...
    res.status(200).send({
      ...
      data: {
        user,
        identityDone: !!user.nom_de_famille && !!user.prenom && !!user.telephone && !!user.addresse_ligne_1 && !!user.code_postal && !!user.ville,
        examinateurDone: !user.roles.includes(UserRoles.CHASSEUR) ? true : !!user.est_forme_a_l_examen_initial && !!user.numero_cfei,
        allEntities,
...
router.get(
  '/entities',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminEntitiesResponse>, next: express.NextFunction) => {
...
router.get(
  '/entity/:entity_id',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminGetEntityResponse>, next: express.NextFunction) => {
        ...
        : await prisma.user.findMany({
            where: {
              roles: (() => {
                ...
                if (entity.type === EntityTypes.ETG || entity.type === EntityTypes.COLLECTEUR_PRO || entity.type === EntityTypes.SVI) {
                  return {
                    has: entity.type,
                  };
                }
              ...
              id: {
                notIn: entity.EntityRelationsWithUsers.filter(
                  (entityRelation) => entityRelation.relation === EntityRelationType.CAN_HANDLE_CARCASSES_ON_BEHALF_ENTITY
...
router.post(
  '/entity/nouvelle',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminNewEntityResponse>, next: express.NextFunction) => {
    const body = req.body;
    ...
    const zacharie_compatible = entityType === EntityTypes.PREMIER_DETENTEUR || entityType === EntityTypes.SVI || entityType === EntityTypes.CCG;
...
router.post(
  '/entity/:entity_id',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminActionEntityResponse>, next: express.NextFunction) => {
    const body = req.body;
...
router.get(
  '/api-key/:api_key_id',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminApiKeyAndApprovalsResponse>, next: express.NextFunction) => {
    const apiKey = await prisma.apiKey.findUnique({
      where: { id: req.params.api_key_id },
      include: {
        approvals: {
    ...
    const allEntities = await prisma.entity.findMany({ where: { deleted_at: null, type: { not: EntityTypes.CCG } } }).then((entities) => {
...
router.get(
  '/official-cfeis',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminOfficialCfeisResponse>, next: express.NextFunction) => {
...
router.get(
  '/carcasses',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminCarcassesResponse>, next: express.NextFunction) => {
...
router.get(
  ...
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminCarcassesIntermediairesResponse>, next: express.NextFunction) => {
    const limit = parseInt(req.query.limit as string) || 100;
    const offset = parseInt(req.query.offset as string) || 0;
...
router.get(
  ...
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminCarcasseDetailResponse>, next: express.NextFunction) => {
...
router.get(
  '/dashboard',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminDashboardResponse>, next: express.NextFunction) => {
...
router.get(
  '/saisies-svi',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminSaisiesSviResponse>, next: express.NextFunction) => {
    ...
    res.status(200).send({
      ...
      data: {
        ...
        taux_mauvaises_pratiques: totalInspectees > 0 ? Math.round((totalMauvaises / totalInspectees) * 1000) / 10 : 0,
      },
      error: '',
    });
  })
...
router.get(
  '/parts-de-marche',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminPartsDeMarcheResponse>, next: express.NextFunction) => {
    const now = dayjs();
    ...
    const rows = await prisma.$queryRaw<Array<{ season_start: number; espece: string; carcasse_count: bigint; nombre_total: bigint }>>`
        SELECT
          CASE
    ...
    const rowsWithCutoff = await prisma.$queryRaw<Array<{ season_start: number; espece: string; carcasse_count: bigint; nombre_total: bigint }>>`
        SELECT
          CASE
            WHEN EXTRACT(MONTH FROM c.date_mise_a_mort) >= 7
            THEN EXTRACT(YEAR FROM c.date_mise_a_mort)::int
...
router.get(
  '/delta-bph',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminDeltaBphResponse>, next: express.NextFunction) => {
...
router.post(
  '/ccg/preview',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminCcgPreviewResponse>, next: express.NextFunction) => {
    ...
    const existingByNumero: Record<string, { nom_d_usage: string; address_ligne_1: string; code_postal: string; ville: string; siret: string }> = {};
...
router.post(
  '/ccg/import',
  passport.authenticate('admin', { session: false }),
  catchErrors(async (req: express.Request, res: express.Response<AdminCcgImportResponse>, next: express.NextFunction) => {
