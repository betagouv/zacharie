...
import { checkApiKeyIsValidMiddleware, getDedicatedEntityLinkedToApiKey, getRequestedUser, mapCarcasseForApi } from '~/utils/api';
...
router.get(
  ...
  catchErrors(async (req: RequestWithApiKey, res: express.Response<CarcasseForResponseForApi>, next: express.NextFunction) => {
    ...
    res.status(200).send({
      ...
      message: 'Pour toute question ou remarque, veuillez contacter le support via le formulaire de contact https://zacharie.beta.gouv.fr/contact.',
    });
  })
);
  ...
  catchErrors(async (req: RequestWithApiKey, res: express.Response<CarcasseForResponseForApi>, next: express.NextFunction) => {
    ...
    switch (entity.type) {
      ...
      case EntityTypes.COLLECTEUR_PRO: {
        const carcasseIntermediaires = fei.CarcasseIntermediaire.filter((c) => c.numero_bracelet === carcasse.numero_bracelet);
...
router.get(
  ...
  passport.authenticate('apiKey', { session: false }),
  checkApiKeyIsValidMiddleware([ApiKeyScope.CARCASSE_READ_FOR_USER]),
  catchErrors(async (req: RequestWithApiKey, res: express.Response<CarcassesForResponseForApi>, next: express.NextFunction) => {
    ...
    res.status(200).send({
      ok: true,
      data: {
        carcasses: carcasses.map((carcasse) => mapCarcasseForApi(carcasse, feis.find((fei) => fei.numero === carcasse.fei_numero)!)),
      },
      message: 'Pour toute question ou remarque, veuillez contacter le support via le formulaire de contact https://zacharie.beta.gouv.fr/contact.',
    });
  })
