  ...
  catchErrors(async (req: RequestWithApiKey, res: express.Response, next: express.NextFunction) => {
    ...
    res.status(200).send({
      ...
      data: {
        ...
        "Paramètre d'URL `contexte` à remplir lors de l'appel à GET /nouvelle-fiche": apiKey.slug_for_context,
      },
      message: `Voici votre clé privée ainsi que le paramètre d'URL \`contexte\` à remplir lors de l'appel à GET /nouvelle-fiche. Veuillez la conserver en toute sécurité. Une fois ouvert, ce lien ne sera plus valide. Si vous avez perdu votre clé et que vous avez besoin d'un nouveau lien, veuillez contacter le support via le formulaire de contact https://zacharie.beta.gouv.fr/contact. `,
    });
  }),
);
