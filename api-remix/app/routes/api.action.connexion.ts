...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (resetPassword === "true") {
    ...
    if (user?.email) {
      ...
      const text = `Bonjour, vous avez demandé à réinitialiser votre mot de passe. Pour ce faire, veuillez cliquer sur le lien suivant : https://api-backup.zacharie.beta.gouv.fr/api/reset-password?token=${token}`;
      sendEmail({
