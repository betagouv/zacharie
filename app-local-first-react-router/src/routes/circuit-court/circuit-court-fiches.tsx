...
export default function CircuitCourtFiches() {
  ...
  useLoaderEffect(() => {
    loadData('circuit-court-fiches').then(() => setIsLoading(false));
  });
  ...
  const [filterStatuses, setFilterStatuses] = useState<TransmissionSimpleStatus[]>(() => {
    try {
      const saved = localStorage.getItem('circuit-court-fiches-filter-statuses');
  ...
  useEffect(() => {
    localStorage.setItem('circuit-court-fiches-filter-statuses', JSON.stringify(filterStatuses));
  }, [filterStatuses]);
  ...
  const [filterPremierDetenteurs, setFilterPremierDetenteurs] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('circuit-court-fiches-filter-premier-detenteurs');
  ...
  useEffect(() => {
    localStorage.setItem(
      'circuit-court-fiches-filter-premier-detenteurs',
  ...
  const [filterSaisons, setFilterSaisons] = useState<number[]>(() => {
    try {
      const saved = localStorage.getItem('circuit-court-fiches-filter-saisons');
  ...
  useEffect(() => {
    localStorage.setItem('circuit-court-fiches-filter-saisons', JSON.stringify(filterSaisons));
  }, [filterSaisons]);

  const [filterDateFrom, setFilterDateFrom] = useState<string>(
    () => localStorage.getItem('circuit-court-fiches-filter-date-from') || ''
  );
  const [filterDateTo, setFilterDateTo] = useState<string>(
    () => localStorage.getItem('circuit-court-fiches-filter-date-to') || ''
  );

  useEffect(() => {
    localStorage.setItem('circuit-court-fiches-filter-date-from', filterDateFrom);
  }, [filterDateFrom]);

  useEffect(() => {
    localStorage.setItem('circuit-court-fiches-filter-date-to', filterDateTo);
  }, [filterDateTo]);

  const [filterCCGs, setFilterCCGs] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('circuit-court-fiches-filter-ccgs');
      if (saved) return JSON.parse(saved) as string[];
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    localStorage.setItem('circuit-court-fiches-filter-ccgs', JSON.stringify(filterCCGs));
  }, [filterCCGs]);

  const [filterCollecteurs, setFilterCollecteurs] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('circuit-court-fiches-filter-collecteurs');
      if (saved) return JSON.parse(saved) as string[];
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    localStorage.setItem('circuit-court-fiches-filter-collecteurs', JSON.stringify(filterCollecteurs));
  }, [filterCollecteurs]);
        ...
        <div className="mx-auto max-w-5xl min-w-0 flex-1 px-4 pt-4 md:px-6">
          ...
          {filteredTransmissions.length > 0 && (
            <CarcassesEspeceSummary
              ...
              storageKey="circuit-court-fiches-espece-summary-open"
            />
          )}
          <FeisWrapper
            viewType={viewType}
      ...
      {paginatedTransmissions.map((transmission) => {
        ...
        return (
          <CardTransmission
            ...
            linkTo={`/app/circuit-court/fei/${getTransmissionLink(transmission)}`}
...
function FeisTableRow({
  ...
  return (
    <tr
      ...
      onClick={() => navigate(`/app/circuit-court/fei/${getTransmissionLink(transmission)}`)}
    >
      <td
        className="px-4 py-3"
