...
export default function FederationCoordonnees({ onboarding = false }: { onboarding?: boolean }) {
  ...
  const handleSubmit = async () => {
    ...
    if (response.ok && response.data?.user?.id) {
      useUser.setState({ user: response.data.user });
      navigate('/app/federation/tableau-de-bord');
    }
  };
        ...
        <div className="fr-col-12 fr-col-md-10 p-4 md:p-0">
          ...
          <div className="fixed bottom-16 left-0 z-50 flex w-full flex-col p-6 pb-2 shadow-2xl md:relative md:bottom-0 md:w-auto md:items-center md:shadow-none">
            <ButtonsGroup
              ...
              buttons={[
                {
                  children: onboarding ? 'Enregistrer et continuer' : 'Enregistrer',
                  type: 'button',
                  nativeButtonProps: {
