...
const carcasseTabs = [
  { tabId: 'identite' as const, label: 'Identité & examen' },
  { tabId: 'detenteurs' as const, label: 'Détenteurs' },
  { tabId: 'svi' as const, label: 'Inspection SVI' },
  { tabId: 'propriete' as const, label: 'Propriété' },
  { tabId: 'timeline' as const, label: 'Fil de vie' },
];

type CarcasseTabId = (typeof carcasseTabs)[number]['tabId'];
    ...
    <>
      ...
      <header className="mt-2 rounded-lg border border-gray-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h2 className="mb-0 text-xl font-bold">Carcasse {carcasse.numero_bracelet}</h2>
            <p className="mb-0 text-sm text-gray-500">
              {carcasse.espece ?? '—'}
              {carcasse.type ? ` · ${carcasse.type}` : ''}
        ...
        <p className="mt-2 mb-0 text-sm text-gray-500">
          FEI&nbsp;:{' '}
          <Link
            to={`/app/admin/fei/${encodeURIComponent(carcasse.fei_numero)}`}
      ...
      <Tabs
        ...
        tabs={carcasseTabs}
        onTabChange={(tabId) => setSelectedTab(tabId as CarcasseTabId)}
      >
              ...
              <Section title="Examen initial (Examinateur)">
                <FieldGrid>
                  ...
                  {carcasse.examinateur_anomalies_carcasse.length > 0 && (
                    <Field label="Anomalies carcasse">
            ...
            {carcasse.premier_detenteur_user_id && (
              <Section title="Premier détenteur">
                <FieldGrid>
                  ...
                  <Field label="Entité dépôt">
                    {carcasse.premier_detenteur_depot_entity_name_cache ?? '—'}
                  </Field>
                  {carcasse.premier_detenteur_depot_ccg_at && (
                    <Field label="Date dépôt CCG">
                      {formatDate(carcasse.premier_detenteur_depot_ccg_at)}
                    </Field>
                  )}
                  ...
                  {carcasse.premier_detenteur_transport_date && (
                    <Field label="Transport date">
                      {formatDate(carcasse.premier_detenteur_transport_date)}
                    </Field>
                  )}
