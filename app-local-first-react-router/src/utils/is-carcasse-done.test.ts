...
describe('isCarcasseDone', () => {
  ...
  it('true for domestic final-consumer usage', () => {
    expect(isCarcasseDone(c({ consommateur_final_usage_domestique: true }))).toBe(true);
  });
    ...
    expect(
      isCarcasseDone(
        c({
          ...
          svi_assigned_to_fei_at: dayjs().subtract(1, 'day').toDate(),
        })
      )
    ).toBe(false);
  });
...
describe('isFeiDone', () => {
  ...
  it('true for domestic final-consumer usage', () => {
    expect(isFeiDone(fei({ consommateur_final_usage_domestique: true }), [c()])).toBe(true);
  });
