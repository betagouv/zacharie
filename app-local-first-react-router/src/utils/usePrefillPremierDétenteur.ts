...
export function usePrefillPremierDétenteurInfos(): PrefillPremierDétenteurInfos | null {
  ...
  const [state] = useState(() => {
    ...
    const match = matchingFeis.find((f) => !!(f.svi_assigned_at || f.intermediaire_closed_at)) || matchingFeis[0];
    if (match) {
      return {
        ...
        premier_detenteur_prochain_detenteur_role_cache: match.premier_detenteur_prochain_detenteur_role_cache,
